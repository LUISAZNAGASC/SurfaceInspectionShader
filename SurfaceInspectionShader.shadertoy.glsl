...
void setEdgeDetectionImageProcessing(inout vec4 fragmentOutputColor, in vec2 fragmentTextureCoordinates)
{
	...
	float edgeDetectionFullIntensity = length(vec2(edgeDetectionXIntensity, edgeDetectionYIntensity).xy);
...
#define NumericConstant_Zero 0000000000.00000000000000000000f
#define NumericConstant_One 0000000001.00000000000000000000f
#define NumericConstant_Epsilon 0000000000.00010000000000000000f
#define NumericConstant_Pi 0000000003.14159265358979323846f
#define NumericConstant_Gamma 0000000000.45454545454545454545f

#define CameraConstant_FarClippingPlane 0000000500.00000000000000000000f

#define SceneConstant_PointOfViewPosition vec3(-6.0f, -6.0f, 24.0f).xyz
...
surfaceData getSphereSurfaceData(const in vec4 sphereRaypointDataPosition, const in transformData sphereTransformData, const in materialData sphereMaterialData)
{
	float sphereSurfaceDataSignedDistance;
	{
		...
		sphereSurfaceDataSignedDistance = length(sphereRaypointDataFullPosition.xyzw) - abs(sphereTransformData.transformDataScale);
	}
...
surfaceData getSceneSurfaceData(const in vec4 sceneRaypointDataPosition)
{
	...
	{
		const float sphereTransformScale = 6.0f;

		vec4 dualSphereRaypointDataFullPosition;
		{
			vec3 dualSphereTransformDataPosition = vec3(0.0f, 0.5f * sphereTransformScale * sin(NumericConstant_Pi * iTime), 0.0f).xyz;

			mat4 dualSphereTransformDataFullTransformation = getTransformDataIdentityTransformation();
			dualSphereTransformDataFullTransformation *= getTransformDataLocalTranslation(SceneConstant_PointOfViewPosition.xyz + dualSphereTransformDataPosition.xyz);
		...
		dualSphereMaterialData.materialDataGeneralColor.rgba = getColorDataDefinedThis(vec3(1.0f, 1.0f, 1.0f).rgb).rgba;
		dualSphereMaterialData.materialDataAmbientAmount = 0.075f;
		dualSphereMaterialData.materialDataDiffuseAmount = 0.75f;
		dualSphereMaterialData.materialDataSpecularAmount = 1.0f;
		dualSphereMaterialData.materialDataSpecularShininess = 0.35f;
		...
		surfaceData leftSphereSurfaceData;
		{
			...
			leftSphereTransformData.transformDataPosition.xyz = vec3(-2.0f / 3.0f * sphereTransformScale, 0.0f, 0.0f).xyz;
			leftSphereTransformData.transformDataRotation.xyz = vec3(0.0f, 0.0f, 0.0f).xyz;
			leftSphereTransformData.transformDataScale = sphereTransformScale;
		...
		surfaceData rightSphereSurfaceData;
		{
			...
			rightSphereTransformData.transformDataPosition.xyz = vec3(2.0f / 3.0f * sphereTransformScale, 0.0f, 0.0f).xyz;
			rightSphereTransformData.transformDataRotation.xyz = vec3(0.0f, 0.0f, 0.0f).xyz;
			rightSphereTransformData.transformDataScale = sphereTransformScale;
		...
		dualSphereSurfaceData = getCombinedSurfaceData(leftSphereSurfaceData, rightSphereSurfaceData, 1.0f / 3.0f * sphereTransformScale);
	}
...
raypointData getSceneCameraRaypointData(const in vec3 sceneCameraTransformDataPosition, const in vec3 sceneLookAtTransformDataPosition, const in vec2 sceneCameraViewportCoordinates, const in float sceneCameraFieldOfView)
{
	...
	vec4 sceneCameraRaypointDataDirection;
	{
		...
		sceneCameraTransformDataFullTransformation = inverse(sceneCameraTransformDataFullTransformation);
...
vec4 getSceneSurfaceColorData(const in raypointData sceneCameraRaypointData, const in surfaceData sceneRaymarchSurfaceData)
{
	...
	vec4 sceneRaymarchMaterialSpecularColor;
	{
		...
		float sceneRaymarchMaterialSpecularShininess = mix(sceneRaymarchMaterialSoftSpecular, sceneRaymarchMaterialSharpSpecular, sceneRaymarchMaterialData.materialDataSpecularShininess);
...
void mainImage(out vec4 fragmentOutputColor, in vec2 fragmentInputCoordinates)
{
	...
	raypointData sceneCameraRaypointData = getSceneCameraRaypointData(vec3(-6.0f, 18.0f, 0.0f).xyz, SceneConstant_PointOfViewPosition.xyz, sceneCameraViewportCoordinates.xy, 30.0f);
